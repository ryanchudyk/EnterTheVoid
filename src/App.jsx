...
const App = () => {
  ...
  const [wordsSinceThreshold, setWordsSinceThreshold] = useState(0);
  ...
  // Progress bar and encouragement opacity (hidden post-threshold unless hovering)
  const progressOpacity = thresholdReached 
    ? (hoveringDone ? 1 : 0) 
    : uiOpacity;
  const encouragementOpacity = thresholdReached 
    ? (hoveringDone ? 1 : 0) 
    : uiOpacity;
  
  // Title stays hidden once it fades out (user has internalized it)
  const titleOpacity = titleHidden ? (hoveringDone ? 0.5 : 0) : uiOpacity;
  
  // Done button opacity - same fade system
  const doneButtonOpacity = hoveringDone ? 1 : calculateOpacity(doneButtonWordsSinceFade);
  
  // Green glow fades after 5 words post-threshold
  const glowOpacity = glowVisible ? Math.max(0, 1 - (wordsSinceThreshold * 0.2)) : 0;
  ...
  useEffect(() => {
    if (stage === 'focus' && isComplete && !thresholdReachedRef.current) {
      ...
      setGlowVisible(true);
      setWordsSinceThreshold(0);
    }
  }, [isComplete, stage]);

  // Track when title should stay hidden
  useEffect(() => {
    if (uiOpacity === 0 && !titleHidden) {
      setTitleHidden(true);
    }
  }, [uiOpacity, titleHidden]);
  ...
  useEffect(() => {
    ...
    if (wordsAdded > 0 && stage === 'focus') {
      ...
      // Track words since threshold for glow fade
      if (thresholdReached) {
        setWordsSinceThreshold(prev => prev + wordsAdded);
        // Also fade the done button when typing
      ...
      // Start new idle timeout (20 seconds)
  ...
  }, [wordCount, thresholdReached, stage]);
